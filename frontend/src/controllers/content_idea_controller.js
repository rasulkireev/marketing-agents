...
export default class extends Controller {
  ...
  createSuggestionHTML(suggestion, contentType) {
          ...
          <div class="mb-4">
            <div class="flex flex-wrap gap-2">
              ${suggestion.target_keywords.map(keyword => `
                <span class="inline-flex items-center px-2 py-1 text-xs font-medium text-gray-600 bg-gray-50 rounded border border-gray-200">
                  ${DOMPurify.sanitize(keyword.trim())}
                </span>
              `).join("")}
            </div>
          </div>
    ...
    const metaDescriptionHTML = suggestion.suggested_meta_description ? `
      <div class="p-3 mb-4 bg-gray-50 rounded-md border border-gray-200">
        <span class="block mb-1 text-xs font-medium text-gray-700">
          Meta Description
        </span>
        <p class="text-sm text-gray-600">
          ${DOMPurify.sanitize(suggestion.suggested_meta_description)}
        </p>
      </div>
    ` : '';
      ...
      >
        <div class="p-6">
          ...
          <div class="flex justify-between items-start mb-4">
            <div class="flex-1">
              <h4 class="mb-2 text-lg font-semibold text-gray-900">
                ${DOMPurify.sanitize(suggestion.title)}
              </h4>
              ...
              <div class="flex gap-3 items-center">
                ...
                <span class="inline-flex items-center px-2 py-1 text-xs font-medium text-gray-700 bg-gray-100 rounded-md">
                  ${DOMPurify.sanitize(suggestion.category)}
                </span>
              </div>
            </div>
          ...
          <p class="mb-4 leading-relaxed text-gray-700">
            ${DOMPurify.sanitize(suggestion.description)}
          </p>
          ...
          <div class="flex justify-between items-center pt-4 border-t border-gray-100">
            ...
            <div>
              <button data-action="archive-suggestion#archive"
                      ...
                      class="inline-flex items-center px-3 py-1.5 text-xs font-medium text-gray-600 rounded border border-gray-300 transition-colors hover:bg-gray-50">
              ...
              <button data-action="archive-suggestion#unarchive"
                      ...
                      class="inline-flex hidden items-center px-3 py-1.5 text-xs font-medium text-gray-600 rounded border border-gray-300 transition-colors hover:bg-gray-50">
                <svg xmlns="http://www.w3.org/2000/svg" class="mr-1 w-3.5 h-3.5" viewBox="0 0 20 20" fill="currentColor">
